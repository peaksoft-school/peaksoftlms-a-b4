...
public class StudentServiceImpl implements StudentService {
    ...
    public StudentResponse saveStudent(StudentRequest request) {
        ...
        checkEmail(email);
        StudentEntity student = studentRepository.save(studentEditMapper.convertToStudent(request));
    ...
    public StudentResponse updateStudent(Long studentId, StudentRequest request) {
        ...
        GroupEntity groupEntity = groupRepository.findById(request.getGroupId()).orElseThrow(() -> {
            ...
            throw new NotFoundException(String.format("Group with id = %s does not exists", request.getGroupId()));
        });
    ...
    public StudentResponse deleteStudent(Long studentId) {
        ...
        if (!existById) {
            ...
            throw new BadRequestException(String.format("Student with id = %s does not exists, you can not delete it", studentId));
        }
    ...
    public StudentResponse setStudentToGroup(Long groupId, Long studentId) {
        GroupEntity group = groupRepository.findById(groupId).orElseThrow(() -> {
            ...
            throw new NotFoundException(String.format("Group with id = %s does not exists", groupId));
        });
        StudentEntity student = getByIdMethod(studentId);
    ...
    public StudentResponse setStudentToCourse(Long courseId, Long studentId) {
        CourseEntity course = courseRepository.findById(courseId).orElseThrow(() -> {
            ...
            throw new NotFoundException(String.format("Course with id = %s does not exists", courseId));
        });
        StudentEntity student = getByIdMethod(studentId);
    ...
    private StudentEntity getByIdMethod(Long studentId) {
        return studentRepository.findById(studentId).orElseThrow(() -> {
            ...
            throw new NotFoundException(String.format("Student with id = %s does not exists", studentId));
        });
    }
